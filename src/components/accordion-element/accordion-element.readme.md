...
`accordion-element` is a collapsible content component that expands and collapses content sections with smooth animations. It features customizable header and content slots, automatic toggle functionality, and emits events when its state changes.
...
| --- | --- | --- |
| `AccordionElementToggle` | `{ open: boolean }` | Fired when the accordion is toggled. The event bubbles up through the DOM. The detail contains the current open state. |
