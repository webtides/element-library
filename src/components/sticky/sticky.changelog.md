...
- BREAKING: renamed the component — tag `el-sticky-element` → `el-sticky`, import subpaths `@webtides/element-library/sticky-element` → `/sticky` (and `/sticky-element/define` → `/sticky/define`), and exported class `StickyElement` → `Sticky`.
- BREAKING: replaced the `.is-sticky`, `.is-up`, `.is-down` host classes with the custom states `:state(sticky)`, `:state(up)`, `:state(down)`. Consumers that styled on the old class names should switch to `el-sticky:state(...) { … }`.
...
- Added `ResizeOberver` to listen for when the size of the `sticky` changes. With this it will now correctly calculate the height of the element even if it contains other custom elements that render after the connected callback.
